...
object Protocol {
  ...
  implicit val oneFrameRateResponseDataDecoder: Decoder[OneFrameRateResponseData] =
    Decoder.forProduct6("from", "to", "bid", "ask", "price", "time_stamp")(
      (from, to, bid, ask, price, timestamp: String) =>
        OneFrameRateResponseData(from, to, bid, ask, price, OffsetDateTime.parse(timestamp))
    )
}
